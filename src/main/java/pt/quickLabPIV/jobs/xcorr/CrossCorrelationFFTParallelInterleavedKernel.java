...
public class CrossCorrelationFFTParallelInterleavedKernel extends Kernel{
	...
	public void run() {
		...
		final int tileNumberJ = j / getLocalSize(0);
		...
		final int xrOffset = 0;
		final int xiOffset = 1;
		final int yrOffset = 2;
		final int yiOffset = 3;
		final int stride = 4;

		final int offset = tileIdx * dimI * dimJ * stride;
		...
		if (lineBlockSize > 1) {
			for (int lineBlockIndex=0; lineBlockIndex < lineBlockSize; lineBlockIndex++) {
				int lineOffsetGlobal = offset + (ilOffset + lineBlockIndex) * dimJ * stride;
				int lineOffsetLocal = threadNrI * dimJ;
				if (tileIdx < numberOfUsedTiles && ilOffset + lineBlockIndex < dimI) {
					//Copy from source array to local buffer...
					for (int indexJ = 0; indexJ < colBlockSize; indexJ++) {
						final int colOffsetGlobal = lineOffsetGlobal + (jlAdj + indexJ) * stride;
						xrLocal[lineOffsetLocal + jlAdj + indexJ] = matrices[colOffsetGlobal + xrOffset];
						xiLocal[lineOffsetLocal + jlAdj + indexJ] = 0.0f;
						yrLocal[lineOffsetLocal + jlAdj + indexJ] = matrices[colOffsetGlobal + yrOffset];
						yiLocal[lineOffsetLocal + jlAdj + indexJ] = 0.0f;
					}
				}
				...
				if (tileIdx < numberOfUsedTiles && ilOffset + lineBlockIndex < dimI) {
					if (lineBlockSize > 1) {
						...
						for (int indexJ = 0; indexJ < colBlockSize; indexJ++) {
							final int colOffsetGlobal = lineOffsetGlobal + (jlAdj + indexJ) * stride;
							matrices[colOffsetGlobal + xrOffset] = xrLocal[lineOffsetLocal + jlAdj + indexJ];
							matrices[colOffsetGlobal + xiOffset] = xiLocal[lineOffsetLocal + jlAdj + indexJ];
							matrices[colOffsetGlobal + yrOffset] = yrLocal[lineOffsetLocal + jlAdj + indexJ];
						    matrices[colOffsetGlobal + yiOffset] = yiLocal[lineOffsetLocal + jlAdj + indexJ];
						}		
					}
				}
				globalBarrier();
			...
			final int lineOffsetGlobal = offset + threadNrI * dimJ * stride;
			final int lineOffsetLocal = threadNrI * dimJ;
			if (tileIdx < numberOfUsedTiles && threadNrI < dimI) {
				//Copy from source array to local buffer...
				for (int indexJ = 0; indexJ < colBlockSize; indexJ++) {
					final int colOffsetGlobal = lineOffsetGlobal + (jlAdj + indexJ) * stride;
					xrLocal[lineOffsetLocal + jlAdj + indexJ] = matrices[colOffsetGlobal + xrOffset];
		...
		for (int lineBlockIndex = 0; lineBlockIndex < lineBlockSize; lineBlockIndex++) {
			...
			if (tileIdx < numberOfUsedTiles && ilOffset + lineBlockIndex < dimI) {				
				if (lineBlockSize == 1) {
					...
					for (int indexJ = 0; indexJ < colBlockSize; indexJ++) {
						int colOffsetGlobal = offset + (jlAdj + indexJ) * dimI * stride + (ilOffset + lineBlockIndex) * stride;
						...
						xrLocal[lineOffsetLocal + jlAdj + indexJ] = matrices[colOffsetGlobal + xrOffset];
						xiLocal[lineOffsetLocal + jlAdj + indexJ] = matrices[colOffsetGlobal + xiOffset];
						yrLocal[lineOffsetLocal + jlAdj + indexJ] = matrices[colOffsetGlobal + yrOffset];
						yiLocal[lineOffsetLocal + jlAdj + indexJ] = matrices[colOffsetGlobal + yiOffset];
					}		
				}
			}
			...
			if (tileIdx < numberOfUsedTiles && ilOffset + lineBlockIndex < dimI) {
				if (lineBlockSize > 1) {
					...
					for (int indexJ = 0; indexJ < colBlockSize; indexJ++) {
						final int colOffsetGlobal = offset + ((jlAdj + indexJ) * dimI + ilOffset + lineBlockIndex)*stride;
						matrices[colOffsetGlobal + xrOffset] = xrLocal[lineOffsetLocal + jlAdj + indexJ];
						matrices[colOffsetGlobal + xiOffset] = xiLocal[lineOffsetLocal + jlAdj + indexJ];
						matrices[colOffsetGlobal + yrOffset] = yrLocal[lineOffsetLocal + jlAdj + indexJ];
						matrices[colOffsetGlobal + yiOffset] = yiLocal[lineOffsetLocal + jlAdj + indexJ];
		...
		for (int lineBlockIndex = 0; lineBlockIndex < lineBlockSize; lineBlockIndex++) {
			final int lineOffsetGlobal = offset + (ilOffset + lineBlockIndex) * dimJ * stride;
			final int lineOffsetLocal = threadNrI * dimJ;
			if (tileIdx < numberOfUsedTiles && ilOffset + lineBlockIndex < dimI) {
				if (lineBlockSize == 1) {
			...
			if (tileIdx < numberOfUsedTiles && ilOffset + lineBlockIndex < dimI) {
				if (lineBlockSize > 1) {
					for (int indexJ = 0; indexJ < colBlockSize; indexJ++) {
						final int colOffsetGlobal = lineOffsetGlobal + (jlAdj + indexJ) * stride;
						matrices[colOffsetGlobal + xrOffset] = xrLocal[threadNrI * dimJ + jlAdj + indexJ];
						matrices[colOffsetGlobal + xiOffset] = xiLocal[threadNrI * dimJ + jlAdj + indexJ];
					}						
				}			
			}
			globalBarrier();
		...
		for (int lineBlockIndex = 0; lineBlockIndex < lineBlockSize; lineBlockIndex++) {
			if (tileIdx < numberOfUsedTiles && ilOffset + lineBlockIndex < dimI) {
				if (lineBlockSize == 1) {			
					...
					for (int indexJ = 0; indexJ < colBlockSize; indexJ++) {
						...
						final int colOffsetGlobal = offset + ((jlAdj + indexJ) * dimI + ilOffset + lineBlockIndex)*stride;
						xrLocal[threadNrI * dimJ + jlAdj + indexJ] = matrices[colOffsetGlobal + xrOffset];
						xiLocal[threadNrI * dimJ + jlAdj + indexJ] = matrices[colOffsetGlobal + xiOffset];
					}
				}
			}
			localBarrier();
			...
			if (tileIdx < numberOfUsedTiles && ilOffset + lineBlockIndex < dimI) {
				...
				for (int indexJ = 0; indexJ < colBlockSize; indexJ++) {
					matrices[offset + ((jlAdj + indexJ) * dimI + ilOffset + lineBlockIndex)*stride] = xrLocal[threadNrI * dimJ + jlAdj + indexJ];
				}
			}
		}
	}
