...
/**
 * Provides unit tests to validate interpolation with a 1D-1D Gaussian function, over a matrix of 7x7 pixels generated by
 * an 2D Symmetric Gaussian function with no noise added.
 * 
 * @author lpnm
 */
public class Gaussian1DInterpolatorAdvanced7PixelsHongWeiGuoTest {
    ...
    public void centeredSigma10() {        
        ...
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM, DIM, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextHongWeiGuo1D(context, DIM);
        ...
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM/2), result.getMainPeakI(), 8e-3f);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM/2), result.getMainPeakJ(), 8e-3f);
    }
    ...
    @Test
    public void deviated01LeftSigma02() {
        float uX = -0.1f;
        float uY = -0.1f;
        float sigmaX = 0.25f;
        float sigmaY = 0.25f;
    ...
    @Test
    public void deviated01RightSigma02() {
        float uX = 0.1f;
        float uY = 0.1f;
        float sigmaX = 0.25f;
        float sigmaY = 0.25f;
