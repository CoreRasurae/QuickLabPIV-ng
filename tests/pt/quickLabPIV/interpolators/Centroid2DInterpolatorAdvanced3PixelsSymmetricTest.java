// SPDX-License-Identifier: GPL-3.0-only
/*
 * QuickLabPIV-ng - A hybrid PIV and PIV software laboratory (new generation)
 *
 * Copyright (C) 2020 Luís Mendes
 */
package pt.quickLabPIV.interpolators;

import static org.junit.Assert.assertEquals;

import java.util.ArrayList;
import java.util.List;

import org.junit.Before;
import org.junit.Test;

import pt.quickLabPIV.Matrix;
import pt.quickLabPIV.PIVContextSingleton;
import pt.quickLabPIV.PIVContextTestsSingleton;
import pt.quickLabPIV.interpolators.IBasicCrossCorrelationInterpolator;
import pt.quickLabPIV.interpolators.ICrossCorrelationInterpolator;
import pt.quickLabPIV.maximum.MaxCrossResult;

/**
 * Provides unit tests to validate interpolation with a Symmetric standard deviation 2D Gaussian function, over a matrix of 3x3 pixels generated by
 * a 2D Symmetric Gaussian function with no noise added.
 * 
 * @author lpnm
 */
public class Centroid2DInterpolatorAdvanced3PixelsSymmetricTest {
    private static final int DIM_X = 3;
    private static final int DIM_Y = DIM_X;

    public IBasicCrossCorrelationInterpolator simpleAdapter(ICrossCorrelationInterpolator interp) {
        return new IBasicCrossCorrelationInterpolator() {
            private ICrossCorrelationInterpolator _interp = interp;
            
            @Override
            public MaxCrossResult interpolate(Matrix m, MaxCrossResult result) {
                List<MaxCrossResult> results = new ArrayList<>(1);
                results.add(result);
                results = _interp.interpolate(results);
                return results.get(0);
            }
        };
    }
    
    @Before
    public void setup() {
        PIVContextTestsSingleton.setSingleton();        
    }
    
    //@Test
    public void centeredSigma10() {
        float uX = 0.0f;
        float uY = 0.0f;
        float sigmaX = 1.0f;
        float sigmaY = 1.0f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }
    
    //Left Sigma 10
    
    //@Test
    public void deviated01LeftSigma10() {
        float uX = -0.1f;
        float uY = -0.1f;
        float sigmaX = 1.0f;
        float sigmaY = 1.0f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated02LeftSigma10() {
        float uX = -0.2f;
        float uY = -0.2f;
        float sigmaX = 1.0f;
        float sigmaY = 1.0f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated03LeftSigma10() {
        float uX = -0.3f;
        float uY = -0.3f;
        float sigmaX = 1.0f;
        float sigmaY = 1.0f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated04LeftSigma10() {
        float uX = -0.4f;
        float uY = -0.4f;
        float sigmaX = 1.0f;
        float sigmaY = 1.0f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated05LeftSigma13() {
        float uX = -0.5f;
        float uY = -0.5f;
        float sigmaX = 1.3f;
        float sigmaY = 1.3f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //Right Sigma 10
    
    //@Test
    public void deviated01RightSigma10() {
        float uX = 0.1f;
        float uY = 0.1f;
        float sigmaX = 1.0f;
        float sigmaY = 1.0f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated02RightSigma10() {
        float uX = 0.2f;
        float uY = 0.2f;
        float sigmaX = 1.0f;
        float sigmaY = 1.0f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated03RightSigma10() {
        float uX = 0.3f;
        float uY = 0.3f;
        float sigmaX = 1.0f;
        float sigmaY = 1.0f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated04RightSigma10() {
        float uX = 0.4f;
        float uY = 0.4f;
        float sigmaX = 1.0f;
        float sigmaY = 1.0f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated05RightSigma13() {
        float uX = 0.5f;
        float uY = 0.5f;
        float sigmaX = 1.3f;
        float sigmaY = 1.3f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //Sigma 05
    
    @Test
    public void centeredSigma05() {
        float uX = 0.0f;
        float uY = 0.0f;
        float sigmaX = 0.5f;
        float sigmaY = 0.5f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }
    
    //Left Sigma 05
    
    @Test
    public void deviated01LeftSigma05() {
        float uX = -0.1f;
        float uY = -0.1f;
        float sigmaX = 0.5f;
        float sigmaY = 0.5f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    @Test
    public void deviated02LeftSigma05() {
        float uX = -0.2f;
        float uY = -0.2f;
        float sigmaX = 0.5f;
        float sigmaY = 0.5f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    @Test
    public void deviated03LeftSigma05() {
        float uX = -0.3f;
        float uY = -0.3f;
        float sigmaX = 0.5f;
        float sigmaY = 0.5f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    @Test
    public void deviated04LeftSigma06() {
        float uX = -0.4f;
        float uY = -0.4f;
        float sigmaX = 0.6f;
        float sigmaY = 0.6f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    @Test
    public void deviated05LeftSigma05() {
        float uX = -0.5f;
        float uY = -0.5f;
        float sigmaX = 0.5f;
        float sigmaY = 0.5f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //Right Sigma 05
    
    @Test
    public void deviated01RightSigma05() {
        float uX = 0.1f;
        float uY = 0.1f;
        float sigmaX = 0.5f;
        float sigmaY = 0.5f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    @Test
    public void deviated02RightSigma05() {
        float uX = 0.2f;
        float uY = 0.2f;
        float sigmaX = 0.5f;
        float sigmaY = 0.5f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    @Test
    public void deviated03RightSigma05() {
        float uX = 0.3f;
        float uY = 0.3f;
        float sigmaX = 0.5f;
        float sigmaY = 0.5f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    @Test
    public void deviated04RightSigma06() {
        float uX = 0.4f;
        float uY = 0.4f;
        float sigmaX = 0.6f;
        float sigmaY = 0.6f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    @Test
    public void deviated05RightSigma05() {
        float uX = 0.5f;
        float uY = 0.5f;
        float sigmaX = 0.5f;
        float sigmaY = 0.5f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    
    //Sigma 01
    
    @Test
    public void centeredSigma01() {
        float uX = 0.0f;
        float uY = 0.0f;
        float sigmaX = 0.206f;
        float sigmaY = 0.206f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }
    
    //Left Sigma 01
    
    //@Test
    public void deviated01LeftSigma01() {
        float uX = -0.1f;
        float uY = -0.1f;
        float sigmaX = 0.206f;
        float sigmaY = 0.206f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated02LeftSigma01() {
        float uX = -0.2f;
        float uY = -0.2f;
        float sigmaX = 0.206f;
        float sigmaY = 0.206f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated03LeftSigma01() {
        float uX = -0.3f;
        float uY = -0.3f;
        float sigmaX = 0.206f;
        float sigmaY = 0.206f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated04LeftSigma01() {
        float uX = -0.4f;
        float uY = -0.4f;
        float sigmaX = 0.206f;
        float sigmaY = 0.206f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated05LeftSigma01() {
        float uX = -0.5f;
        float uY = -0.5f;
        float sigmaX = 0.206f;
        float sigmaY = 0.206f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //Right Sigma 01
    
    //@Test
    public void deviated01RightSigma01() {
        float uX = 0.1f;
        float uY = 0.1f;
        float sigmaX = 0.206f;
        float sigmaY = 0.206f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
       assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated02RightSigma01() {
        float uX = 0.2f;
        float uY = 0.2f;
        float sigmaX = 0.206f;
        float sigmaY = 0.206f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated03RightSigma01() {
        float uX = 0.3f;
        float uY = 0.3f;
        float sigmaX = 0.206f;
        float sigmaY = 0.206f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated04RightSigma01() {
        float uX = 0.4f;
        float uY = 0.4f;
        float sigmaX = 0.206f;
        float sigmaY = 0.206f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated05RightSigma01() {
        float uX = 0.5f;
        float uY = 0.5f;
        float sigmaX = 0.206f;
        float sigmaY = 0.206f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    
    /****************************
     * Assymetric deviation
     ****************************/
       
    //Left, Bottom Sigma 10
    
    //@Test
    public void deviated01LeftBottomSigma10() {
        float uX = -0.1f;
        float uY = +0.1f;
        float sigmaX = 1.0f;
        float sigmaY = 1.0f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated02LeftBottomSigma10() {
        float uX = -0.2f;
        float uY = +0.2f;
        float sigmaX = 1.0f;
        float sigmaY = 1.0f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated03LeftBottomSigma10() {
        float uX = -0.3f;
        float uY = +0.3f;
        float sigmaX = 1.0f;
        float sigmaY = 1.0f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated04LeftBottomSigma10() {
        float uX = -0.4f;
        float uY = +0.4f;
        float sigmaX = 1.0f;
        float sigmaY = 1.0f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated05LeftBottomSigma13() {
        float uX = -0.5f;
        float uY = +0.5f;
        float sigmaX = 1.3f;
        float sigmaY = 1.3f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //Right Bottom Sigma 10
    
    //@Test
    public void deviated01RightBottomSigma10() {
        float uX = 0.1f;
        float uY = -0.1f;
        float sigmaX = 1.0f;
        float sigmaY = 1.0f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated02RightBottomSigma10() {
        float uX = 0.2f;
        float uY = -0.2f;
        float sigmaX = 1.0f;
        float sigmaY = 1.0f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated03RightBottomSigma10() {
        float uX = 0.3f;
        float uY = -0.3f;
        float sigmaX = 1.0f;
        float sigmaY = 1.0f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated04RightBottomSigma10() {
        float uX = 0.4f;
        float uY = -0.4f;
        float sigmaX = 1.0f;
        float sigmaY = 1.0f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated05RightBottomSigma13() {
        float uX = 0.5f;
        float uY = -0.5f;
        float sigmaX = 1.3f;
        float sigmaY = 1.3f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //Sigma 05    
    //Left Bottom Sigma 05

    @Test
    public void deviated01BottomSigma05() {
        float uX = 0.0f;
        float uY = 0.1f;
        float sigmaX = 0.5f;
        float sigmaY = 0.5f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
       assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    
    @Test
    public void deviated01LeftBottomSigma05() {
        float uX = -0.1f;
        float uY = 0.1f;
        float sigmaX = 0.5f;
        float sigmaY = 0.5f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
       assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    @Test
    public void deviated02LeftBottomSigma05() {
        float uX = -0.2f;
        float uY = 0.2f;
        float sigmaX = 0.5f;
        float sigmaY = 0.5f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    @Test
    public void deviated03LeftBottomSigma05() {
        float uX = -0.3f;
        float uY = 0.3f;
        float sigmaX = 0.5f;
        float sigmaY = 0.5f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    @Test
    public void deviated04LeftBottomSigma06() {
        float uX = -0.4f;
        float uY = 0.4f;
        float sigmaX = 0.6f;
        float sigmaY = 0.6f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    @Test
    public void deviated05LeftBottomSigma05() {
        float uX = -0.5f;
        float uY = 0.5f;
        float sigmaX = 0.5f;
        float sigmaY = 0.5f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //Right Sigma 05
    
    @Test
    public void deviated01RightBottomSigma05() {
        float uX = 0.1f;
        float uY = -0.1f;
        float sigmaX = 0.5f;
        float sigmaY = 0.5f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    @Test
    public void deviated02RightBottomSigma05() {
        float uX = 0.2f;
        float uY = -0.2f;
        float sigmaX = 0.5f;
        float sigmaY = 0.5f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    @Test
    public void deviated03RightBottomSigma05() {
        float uX = 0.3f;
        float uY = -0.3f;
        float sigmaX = 0.5f;
        float sigmaY = 0.5f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    @Test
    public void deviated04RightBottomSigma06() {
        float uX = 0.4f;
        float uY = -0.4f;
        float sigmaX = 0.6f;
        float sigmaY = 0.6f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    @Test
    public void deviated05RightBottomSigma05() {
        float uX = 0.5f;
        float uY = -0.5f;
        float sigmaX = 0.5f;
        float sigmaY = 0.5f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    
    //Sigma 01
        
    //Left Bottom Sigma 01
    
    //@Test
    public void deviated01LeftBottomSigma01() {
        float uX = -0.1f;
        float uY = 0.1f;
        float sigmaX = 0.206f;
        float sigmaY = 0.206f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated02LeftBottomSigma01() {
        float uX = -0.2f;
        float uY = 0.2f;
        float sigmaX = 0.206f;
        float sigmaY = 0.206f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated03LeftBottomSigma01() {
        float uX = -0.3f;
        float uY = 0.3f;
        float sigmaX = 0.206f;
        float sigmaY = 0.206f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated04LeftBottomSigma01() {
        float uX = -0.4f;
        float uY = 0.4f;
        float sigmaX = 0.206f;
        float sigmaY = 0.206f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated05LeftBottomSigma01() {
        float uX = -0.5f;
        float uY = 0.5f;
        float sigmaX = 0.206f;
        float sigmaY = 0.206f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //Right Bottom Sigma 01
    
    //@Test
    public void deviated01RightBottomSigma01() {
        float uX = 0.1f;
        float uY = -0.1f;
        float sigmaX = 0.206f;
        float sigmaY = 0.206f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated02RightBottomSigma01() {
        float uX = 0.2f;
        float uY = -0.2f;
        float sigmaX = 0.206f;
        float sigmaY = 0.206f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated03RightBottomSigma01() {
        float uX = 0.3f;
        float uY = -0.3f;
        float sigmaX = 0.206f;
        float sigmaY = 0.206f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated04RightBottomSigma01() {
        float uX = 0.4f;
        float uY = -0.4f;
        float sigmaX = 0.206f;
        float sigmaY = 0.206f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

    //@Test
    public void deviated05RightBottomSigma01() {
        float uX = 0.5f;
        float uY = -0.5f;
        float sigmaX = 0.206f;
        float sigmaY = 0.206f;
        PIVContextSingleton context = PIVContextSingleton.getSingleton();
        Matrix matrix = GaussianTestHelper.createMatrixWithGaussian2DAssymmetric(DIM_X, DIM_Y, uX, sigmaX, uY, sigmaY);
        MaxCrossResult result = GaussianTestHelper.setupPIVContextCentroid2D(context, DIM_X);
        result.setCrossMatrix(matrix);
        
        IBasicCrossCorrelationInterpolator interpolator = simpleAdapter(context.getPIVReusableObjects().getOrCreateInterpolator());
        result = interpolator.interpolate(matrix, result);
        System.out.println(result);      
        
        assertEquals("Interpolation incorrectly deviated in I direction from integer max.", uY + (float)(DIM_Y/2), result.getMainPeakI(), 5e-2);
        assertEquals("Interpolation incorrectly deviated in J direction from integer max.", uX + (float)(DIM_X/2), result.getMainPeakJ(), 5e-2);
    }

}
