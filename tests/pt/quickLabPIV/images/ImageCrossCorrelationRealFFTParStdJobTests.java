...
public class ImageCrossCorrelationRealFFTParStdJobTests {
	...
	private static boolean dumpXCorr = true;
	...
	public void testSyntheticImageCrossCorrelationTopLeftPass(int height, int width, ComputationDevice device) {
		...
		CrossCorrelationRealFFTParStdJob job = new CrossCorrelationRealFFTParStdJob(device, false, inputMatricesF, inputMatricesG);
	    /*if (DeviceManager.getSingleton().getJTP().equals(device)) {
        ...
        for (int i = 0; i < 2*height-1; i++) {
            for (int j = 0; j < 2*width-1; j++) {
              assertEquals("Cross correlation doesn't match expected at [i=" + i + ", j=" + j + "]", 
                      resultMat[i][j]/maxValueValidation*maxValueComputed, resultCL[i][j], 1.0f);  
            }
        }
	...
	public void testSyntheticImageCrossCorrelationBottomRightPass(int height, int width, ComputationDevice device) {
		...
		/*KernelManager.setKernelManager(new KernelManager() {
		    ...
		    protected LinkedHashSet<Device> createDefaultPreferredDevices() {		        
		        ...
		        devices.add(DeviceManager.getSingleton().getJTP().getAparapiDevice());
		        return devices;
		    }
		});
		device = DeviceManager.getSingleton().getJTP();
		device.setMinGroupThreads(2);*/
		CrossCorrelationRealFFTParStdJob job = new CrossCorrelationRealFFTParStdJob(device, false, inputMatricesF, inputMatricesG);
		//job.setEmulationMode(EmulationModeEnum.GPU);
		try {
	        job.analyze();
